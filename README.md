...
参考 lh_host_computer 前端惯例（任务分组 tab + 工作流引导头 + 稳定对象名）编排，降低单屏控件密度、按工作流排序：

- **左侧 3D 视图**：顶部常驻**相机工具条**（等轴/俯视/重置视角 + 操作提示"左键旋转/右键平移/滚轮缩放；旁观者视角滚轮=调离车距离"），下方渲染器
- **右侧面板**：顶部**快速上手引导条**（① 数据加载目录 → ② 回放播放 → ③ 换视角/滚轮观察），下方 4 个任务 tab：
  - **数据**：加载目录/加载文件/配准/步长/导出 PCD/已加载信息 + 多目录叠加 + 合成测试场景
  - **回放**：真实序列回放（加载序列/播放/停止/速度/视角/进度）+ 合成实时流
  - **显示**：着色/点大小/体素降采样/点数预算/EDL + 图层可见性（全局/当前帧/车模型）
  - **工具**：GPU 剖面（法向+原点滑块）+ 距离测量 + 点拾取
- **底部状态栏**：FPS / 点数 / 内存 + 操作反馈消息

常用操作（相机、播放）始终可见或一步可达；不常用参数收进对应 tab，关键控件带 tooltip 说明。
...
- [ ] **T8e 导出合并点云为 PCD**：加载文件/目录后点"导出合并点云为 PCD"，把当前合并（配准后）点云写成单个完整 .pcd（binary 默认、含 intensity 则带强度字段；也支持 ascii）。默认文件名 `<目录名>_merged.pcd`。导出即当前加载结果（含步长/预算效果）；若要原始全密度，加载时设步长 1 并取消点数预算。实测 company_wai 575,958 点导出 8.8MB，读回点数/坐标/强度逐点一致
