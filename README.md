...
- [ ] **T3c 跟随小车视角**：回放时勾选"跟随小车视角"，相机用 alidarState.txt 位姿（位置+四元数朝向）置于车后上方、注视车前方，并按 alpha=0.25 平滑插值消除抖动；取消勾选则相机固定（可自由旋转观察）。实测 company_wai12 车移动 23m 时相机跟随 20.4m、相机-车距离稳定 13~16m、相邻帧位移平滑无跳变
- [ ] **T4 GPU 剖面**：勾选"启用剖面"，拖动三个滑块调整法向和原点，确认点云被裁剪且 FPS 不掉（GPU 侧裁剪）
- [ ] **T5 距离测量**：勾选"测量工具"，在点云上点两点，观察顶部标题栏显示距离
- [ ] **T6 点拾取**：勾选"点拾取"，点击点云任意点，状态栏显示坐标
- [ ] **T7 车模型叠加**：勾选"显示 AGV 车模型"，确认蓝色立方体渲染正常，随视角一起变换
