...
- [ ] **T3c 回放视角切换面板**："回放视角"下拉四选一，均用 alidarState.txt 位姿驱动、平滑插值消抖：①**车后跟随**（车后 12m 上 7m 注视车前）；②**旁观者**（斜向侧俯视跟随，右侧 18m+上 12m+稍后 6m，离车约 23m）；③**俯视跟随**（车正上方 45m 往下看、跟随位置、北向上）；④**上帝视角**（固定全局俯瞰不跟车，切换时立即 fit 全场景）。实测 company_wai12：third 相机-车 15.8m / observer 23.1m（侧 17.7+上 12+后 8.8）/ top 正上方 45m 水平偏移 1.7m / god 全程位移 0
